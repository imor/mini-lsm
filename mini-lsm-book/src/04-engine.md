...
Before implementing put and delete, let's revisit how the LSM tree works. The structure of LSM includes:
...
- SSTs: mem-tables can be flushed to disk in SST format. SSTs are organized in multiple levels.

In this part, we only need to take the lock and write the entry (or tombstone) into the active mem-table. You can modify
`lsm_storage.rs`.

## Task 2 - Get

To get a value from the LSM, we simply probe the active memtable, immutable memtables (from latest to earliest),
and all the SSTs. To reduce the critical section, we can hold the read lock to copy all the pointers to mem-tables and
SSTs out of the `LsmStorageInner` structure, and create iterators outside the critical section. Be careful about the
order when creating iterators and probing.

## Task 3 - Scan

To create a scan iterator `LsmIterator`, you need to use `TwoMergeIterator` to merge `MergeIterator` on the mem-table
...
The sequence of key-value pairs produced by `TwoMergeIterator` may contain empty values, which means that the value is
deleted. `LsmIterator` should filter these empty values. It also needs to correctly handle the start and end bounds.

## Task 4 - Sync

In this part, we implement mem-tables and flush to L0 SSTs in `lsm_storage.rs`. As in task 1, write operations go
directly into the active mutable mem-table. Once `sync` is called, we flush SSTs to disk in two steps:

- First, move the current mutable mem-table to the immutable mem-table list, so that no future requests will go into the
...
Now that we have implemented the LSM structure, we can start writing to disk! Previously in `table.rs`,
we implemented a `FileObject` struct without writing anything to disk. In this task, we will change the implementation
so that:

- `read` will read from disk without any caching using `read_exact_at` in `std::os::unix::fs::FileExt`.
- The size of the file should be stored inside the struct, and the `size` function directly returns it.
- `create` should write the file to disk. Generally you should call `fsync` on that file, but this would slow down
  unit tests significantly. Therefore, we don't do fsync until day 6 recovery.
- `open` remains unimplemented until day 6 recovery.

After that, we can implement a new `read_block_cached` function on `SsTable` to leverage block cache for
serving read requests. Upon initializing the `LsmStorage` struct, you should create a block cache of 4GB size using
`moka-rs`. Blocks are cached by SST id + block id. Use `try_get_with` to get the block from cache or populate the cache
if there's a cache miss. If there are multiple requests reading the same block and the cache misses, `try_get_with` will only issue a
single read request to disk and broadcast the result to all requests.
