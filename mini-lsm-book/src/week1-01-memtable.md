...
First, let us implement the in-memory structure of an LSM storage engine: the memtable. We choose [crossbeam's skiplist implementation](https://docs.rs/crossbeam-skiplist/latest/crossbeam_skiplist/) as the data structure for the memtable because it supports lock-free concurrent reads and writes. We will not cover in depth how a skiplist works, but in a nutshell, it is an ordered key-value map that easily allows concurrent reads and writes.

crossbeam-skiplist provides similar interfaces to the Rust std's `BTreeMap`: insert, get, and iter. The only difference is that the modification interfaces (such as `insert`) only require an immutable reference to the skiplist, instead of a mutable one. Therefore, in your implementation, you should not take any mutex when implementing the memtable structure.
...
We use the `bytes` crate for storing data in the memtable. `bytes::Bytes` is similar to `Arc<[u8]>`. When you clone `Bytes` or get a slice of `Bytes`, the underlying data is not copied, and therefore cloning is cheap. Instead, it simply creates a new reference to the storage area, and the storage area will be freed when there are no references to that area.
...
Now, we will add our first data structure, the memtable, to the LSM state. In `LsmStorageState::create`, you will find that when an LSM structure is created, we initialize a memtable with id 0. This is the **mutable memtable** in the initial state. At any point in time, the engine will have only one mutable memtable. A memtable usually has a size limit (such as 256MB), and it will be frozen to an immutable memtable when it reaches the size limit.

Looking at `lsm_storage.rs`, you will find there are two structures that represent a storage engine: `MiniLSM` and `LsmStorageInner`. `MiniLSM` is a thin wrapper for `LsmStorageInner`. You will implement most of the functionalities in `LsmStorageInner`, until week 2 compaction.
...
Your `delete` implementation should simply put an empty slice for that key, which we call a **delete tombstone**. Your `get` implementation should handle this case accordingly.
...
2. `RwLock<Arc<LsmStorageState>>`: This read-write lock protects the _pointer_ to the current `Arc<LsmStorageState>` (the active snapshot).
   ...
   - **Writers** (when modifying the state, e.g., freezing a memtable) will:
     - Create a _new_ `LsmStorageState` instance, often by cloning the data from the current snapshot and then applying modifications.
...
Because there could be multiple threads putting data into the storage engine, `force_freeze_memtable` might be called concurrently from multiple threads. You will need to think about how to avoid race conditions in this case.
...
...where you modify everything while holding the LSM state's write lock.

This works fine for now. However, consider the case where you want to create a write-ahead log file for every memtable you have created.
...
Now when we freeze the memtable, no other threads can have access to the LSM state for several milliseconds, which creates a spike of latency.
...
Then, we do not have costly operations within the state write lock region. Now, consider the case where the memtable is about to reach the capacity limit and two threads successfully put two keys into the memtable, both discovering that the memtable has reached the capacity limit after putting the two keys. Both will perform a size check on the memtable and decide to freeze it. In this case, we might create one empty memtable that is then immediately frozen.
...
You will see this pattern often in future chapters. For example, for L0 flush,
...
* Is the memory layout of the memtable efficient / does it have good data locality? (Think of how `Bytes` is implemented and stored in the skiplist...) What are the possible optimizations to make the memtable more efficient?
...
* After freezing the memtable, is it possible that some threads still hold the old LSM state and write into these immutable memtables? How does your solution prevent it from happening?
