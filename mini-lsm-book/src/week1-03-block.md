...
You have already implemented all in-memory structures for an LSM storage engine in the previous two chapters. Now it's time to build the on-disk structures. The basic unit of the on-disk structure is blocks. Blocks are usually 4KB in size (the size may vary depending on the storage medium), which is equivalent to the page size in the operating system and the page size on an SSD. A block stores ordered key-value pairs. An SST is composed of multiple blocks. When the number of memtables exceeds the system limit, the engine flushes the memtable as an SST. In this chapter, you will implement the encoding and decoding of a block.
...
Now that we have an encoded block, we need to implement the `BlockIterator` interface so that users can look up or scan keys in the block.
...
The iterator should copy the `key` from the block and store it inside the iterator (we will have key compression in the future and you will need to do so). For the value, you should only store the begin/end offset in the iterator without copying it.
...
- Is your implementation prone to a maliciously-built block? Will there be invalid memory access, or OOMs, if a user deliberately constructs an invalid block?
