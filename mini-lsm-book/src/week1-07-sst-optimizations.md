...
Bloom filters are probabilistic data structures that maintain a set of keys. You can add keys to a bloom filter, and you can determine what key may exist or must not exist in the set of keys being added to the bloom filter.
...
For `e -> 3 2`, since bit 2 is not set, it should not be in the original set. For `g -> 1 3`, because both bits are set, it may or may not exist in the set. For `h -> 0 0`, both bits (actually it's one bit) are not set, and therefore it should not be in the original set.
...
Remember that at the end of the last chapter, we implemented SST filtering based on key range. Now, on the `get` read path, we can also use the bloom filter to ignore SSTs that do not contain the key that the user wants to look up, therefore reducing the number of files to be read from disk.
...
In the implementation, you will build a bloom filter from key hashes (which are u32 numbers). For each hash, you will need to set `k` bits. The bits are computed by:
...
We use the `farmhash` crate to compute the hashes of the keys. When building the SST, you also need to build the bloom filter by computing the key hash using `farmhash::fingerprint32`. You will need to encode/decode the bloom filters with the block meta. You can choose a false positive rate of 0.01 for your bloom filter. You may need to add new fields to the structures apart from the ones provided in the starter code as necessary.

After that, you can modify the `get` read path to filter SSTs based on bloom filters.

We do not have an integration test for this part and you will need to ensure that your implementation still passes all previous chapter tests.
...
As the SST file stores keys in order, it is possible that the user stores keys of the same prefix, and we can compress the prefix in the SST encoding to save space.
